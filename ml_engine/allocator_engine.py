...
def run_allocation():
    ...
    pairs = [(s["id"], i["id"], scores[s["id"]][i["id"]]) for s in students for i in internships]
    ...
    for student_id, internship_id, score in pairs:
        if student_id in allocated_students:
            continue
        internship_slots = next(i["slots"] for i in internships if i["id"] == internship_id)
        if len(allocations[internship_id]) < internship_slots:
    ...
    return scores, allocations, unallocated
